...
const ReadingHistoryPanel = lazy(() => import('./components/ReadingHistoryPanel'));
...
function detectLowPerformanceDevice() {
  ...
  const slowNetwork = navigator.connection?.saveData === true || /2g/.test(navigator.connection?.effectiveType || '');
  return Boolean(reducedMotion || lowMemory || lowCpu || slowNetwork);
}

export default function App() {
  const { state, dispatch, set } = useApp();
  const { splashDone, lang, sidebarOpen, displayMode, currentSurah, currentPage, currentJuz } = state;
      ...
      case 'ArrowLeft':
        ...
        if (displayMode === 'page') {
          ...
          if (lang === 'ar' ? currentSurah > 1 : currentSurah < 114)
            dispatch({ type: 'NAVIGATE_SURAH', payload: { surah: lang === 'ar' ? currentSurah - 1 : currentSurah + 1 } });
        }
        break;
      case 'ArrowRight':
        e.preventDefault();
  ...
  }, [displayMode, currentSurah, currentPage, currentJuz, lang, sidebarOpen, state.searchOpen, state.settingsOpen, state.bookmarksOpen, dispatch, set]);
  ...
  if (!splashDone) {
    return (
      <SplashScreen
        ...
        onPrefetch={() => prefetchInitialData(state.currentSurah, state.riwaya, state.translationLang)}
